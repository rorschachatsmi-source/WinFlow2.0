...
class TestJobDeps(unittest.TestCase):
    ...
    def test_link_and_unlink_jobs(self):
        ...
        self.assertIn("APR/apr/" + jobs[1], doc.get_job(parent)[2]["children"])
    ...
    def test_set_job_parents_multiple(self):
        ...
        parents = get_parent_keys(doc, j2)
        self.assertEqual(set(parents), {j0, j1})
        self.assertEqual(set(get_file_parent_keys(doc, j2)), {j0, j1})

    def test_rejects_cycle(self):
        from flow_graph import annotate_job_relations
        ...
        annotate_job_relations(doc.stages)
        a = _job_key("s1", "t1", "a")
        c = _job_key("s1", "t1", "c")
    ...
    def test_link_creates_dummy_outputs_when_missing(self):
        ...
        self.assertIn(a, get_parent_keys(doc, b))
    ...
    def test_link_merges_into_child_task(self):
        ...
        self.assertIn(new_a, get_parent_keys(doc, new_b))
        graph = build_job_graph(doc)
    ...
    def test_link_merges_into_child_stage(self):
        ...
        self.assertIsNotNone(doc.get_job(new_a))
        self.assertIsNone(doc.get_job(a))
    ...
    def test_link_later_job_as_parent_of_earlier_root(self):
        """A root job can receive a later same-task job as parent."""
        doc = FlowDocument(
            stages=[
                make_stage(
                    "s1",
        ...
        a = _job_key("s1", "t1", "a")
        b = _job_key("s1", "t1", "b")
        # make_job starts with empty relations — no task-order edge until seeded.
        self.assertEqual(doc.get_job(a)[2].get("parents"), [])
        self.assertFalse(would_create_cycle(doc, b, a))

        err, new_a, _notes = link_jobs(doc, b, a)
        self.assertIsNone(err, err)
        self.assertIn(b, get_parent_keys(doc, new_a))
