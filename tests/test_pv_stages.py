...
class TestPVStages(unittest.TestCase):
    ...
    def test_verify_stage_omitted_when_flags_off(self):
        self.assertIsNone(verify_stage({}, sample_config()))

    def test_verify_stage_includes_enabled_checks(self):
        settings = {"FLAG_DRCBE": "1", "FLAG_DRCFE": "1"}
        stage = verify_stage(settings, sample_config())
        self.assertIsNotNone(stage)
        task_names = [task["name"] for task in stage["tasks"]]
        self.assertEqual(task_names, ["DRC_BE", "DRC_FE"])
